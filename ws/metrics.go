...
func (m *metrics) RecordMessageRate(ctx context.Context, direction, chargePointId string) {
	m.messageRate.Record(context.Background(), 1)
}

func (m *metrics) RecordPingPongDuration(ctx context.Context, duration time.Duration, chargePointId string) {
	m.pingPongDurationMetric.Record(ctx, duration.Seconds())
}
