...
// Now creates a new DateTime struct, containing a time.Now() value.
func Now() *DateTime {
...
// FormatTimestamp formats the UTC timestamp using the DateTimeFormat setting.
...
// FormatTimestamp formats the given time using the DateTimeFormat setting.
func FormatTimestamp(t time.Time) string {
	return t.UTC().Format(DateTimeFormat)
}

// DateTimeIsNull checks if a DateTime pointer is non-nil but contains a zero time.
