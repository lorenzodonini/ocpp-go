...
func (cs *chargingStation) GetCertificateChainStatus(info []iso15118.CertificateStatusRequestInfoType, props ...func(request *iso15118.GetCertificateChainStatusRequest)) (*iso15118.GetCertificateChainStatusResponse, error) {
	request := iso15118.NewGetCertificateChainStatusRequest(info)
	for _, fn := range props {
		fn(request)
	}
	response, err := cs.SendRequest(request)
...
func (cs *chargingStation) NotifyPriorityCharging(transactionId string, activated bool, props ...func(request *smartcharging.NotifyPriorityChargingRequest)) (*smartcharging.NotifyPriorityChargingResponse, error) {
	request := smartcharging.NewNotifyPriorityChargingRequest(transactionId, activated)
	for _, fn := range props {
		fn(request)
	}
	response, err := cs.SendRequest(request)
	...
	return response.(*smartcharging.NotifyPriorityChargingResponse), err
}

func (cs *chargingStation) NotifySettlement(transactionId string, paymentReference string, status tariffcost.PaymentStatusEnumType, settlementAmount float64, settlementTime types.DateTime, props ...func(request *tariffcost.NotifySettlementRequest)) (*tariffcost.NotifySettlementResponse, error) {
	request := tariffcost.NewNotifySettlementRequest(transactionId, paymentReference, status, settlementAmount, settlementTime)
	for _, fn := range props {
		fn(request)
	}
	response, err := cs.SendRequest(request)
	...
	return response.(*tariffcost.NotifySettlementResponse), err
}

func (cs *chargingStation) NotifyWebPaymentStarted(evseId int, timeout int, props ...func(request *tariffcost.NotifyWebPaymentStartedRequest)) (*tariffcost.NotifyWebPaymentStartedResponse, error) {
	request := tariffcost.NewNotifyWebPaymentStartedRequest(evseId, timeout)
	for _, fn := range props {
		fn(request)
	}
	response, err := cs.SendRequest(request)
	...
	return response.(*tariffcost.NotifyWebPaymentStartedResponse), err
}
