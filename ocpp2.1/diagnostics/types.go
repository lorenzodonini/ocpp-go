...
const (
	...
	MonitorTargetDelta          MonitorType = "TargetDelta"          // Triggers an event notice when the actual value differs from the target value more than plus or minus value since the time that this monitor was set or since the last time this event notice was sent, whichever was last.
	MonitorTargetDeltaRelative  MonitorType = "TargetDeltaRelative"  // Triggers an event notice when the actual value differs from the target value more than plus or minus (value * target value) since the time that this monitor was set or since the last time this event notice was sent, whichever was last.
)
...
func isValidMonitorType(fl validator.FieldLevel) bool {
	...
	case MonitorUpperThreshold, MonitorLowerThreshold, MonitorDelta, MonitorPeriodic, MonitorPeriodicClockAligned,
		MonitorTargetDelta, MonitorTargetDeltaRelative:
		return true
	default:
		return false
	}
...
func init() {
	_ = types.Validate.RegisterValidation("monitorType21", isValidMonitorType)
}
